...
User Function ADPLA04b()
	...
	Local cNatu		:= SuperGetMV('MS_NATUREZ', .F., 'DIV')
	...
	If ZZ4->ZZ4_STATUS == 'A'
		...
		If MsgYesNo('Confirma a efetivacao?')
			...
			Else
				...
				ApMsgInfo("Titulo incluido com sucesso!", 'Atencao')
			Endif

		EndIf

	Else
		MsgAlert('Somente efetivar um movimento ABERTO. ')
	EndIf
...
User Function ADPLA04c()
	...
	If ZZ4->ZZ4_STATUS $ 'A,E' // Somente aberto ou efetivado
		...
		If MsgYesNo('Confirma a cancelamento?')
			...
			If ZZ4->ZZ4_STATUS == 'E'
				...
				If SE2->(DbSeek(xFilial('SE2') + ZZ4->ZZ4_CODIGO)) // Se encontrou o titulo a ser cancelado
					...
					MsExecAuto( { |x,y,z| FINA050(x,y,z)}, aArray,,5)  // 3 - Inclusao, 4 - Alteracao, 5 - Exclusao
					...
					Else
						...
						ApMsgInfo("Titulo excluido do com sucesso!", 'Atencao')
					Endif
